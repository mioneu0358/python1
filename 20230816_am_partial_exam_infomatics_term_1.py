...
def pour(p,c,x,y):
# 𝑖) 시험관의 상태 𝑝, 𝑖𝑖) 시험관의 용량 𝑐, 𝑖𝑖𝑖) 소스 시험관 번호 𝑥, 𝑖𝑣) 목적지 시험관 번호 𝑦
    if 상태 p에있는 소스 x에서 y로 못부으면:
        return None
    소스 x에서 y로 붓는다
    return 시험관
# 함수 작성: next_states 함수는 시험관의 상태 𝑝와 시험관의 용량 𝑐을 입력으로 받습니다. 이 함수는 한 시험관에서 다른 시험관으로 액체를 부은 후 따라올 수 있는 모든 상태를 포함하는 집합을 반환해야 합니다.
def next_states(p,c):
    return {시험관에서 다른 시험관으로 액체를 부은 후 따라올 수 있는 모든 상태를 포함하는 집합}
...
# 함수 작성: path 함수는 시험관의 상태 𝑝와 선행자가 포함된 딕셔너리 𝑃(dict)를 입력으로 받습니다. 딕셔너리 𝑃는 시험관 퍼즐을 해결하기 위해 표준 절차를 적용하는 동안의 한 시점에서 선행자를 설명하며,
# 그 순간에 상태 𝑝는 이전에 큐에 추가되었습니다. 이 함수는 초기 상태에서 상태 𝑝까지의 경로를 설명하는 상태 목록을 반환해야 합니다. 함수는 딕셔너리 𝑃를 수정해서는 안 됩니다.

# 함수 작성: solution 함수는 시험관 퍼즐의 일반화된 버전을 설명하는 세 개의 인수를 입력으로 받습니다: 𝑖) 시험관의 초기 상태 𝑝, 𝑖𝑖) 시험관의 용량 𝑐, 𝑖𝑖𝑖) 요청된 액체의 양 𝑣∈ℕ0 (정수).
...
[(3, 0, 5), (3, 2, 3), (3, 5, 0)]
print(predecessor)
{(0, 0, 8): None, (0, 5, 3): (0, 0, 8), (3, 0, 5): (0, 0, 8), (3, 2, 3): (0, 5, 3)
    , (3, 5, 0): (0, 5, 3)}
...
[(0, 0, 0, 0, 13), (0, 0, 0, 11, 2), (5, 0, 0, 6, 2), (0, 0, 0, 6, 7), (5, 0, 0, 1
                                                                        , 7), (0, 0, 0, 1, 12)]
