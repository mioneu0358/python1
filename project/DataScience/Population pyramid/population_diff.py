...
new_columns = []
for col in df_m_2023.columns:
    new_columns.append(col[:-2])
df_m_2023.columns = new_columns

# 데이터 타입을 정수로 변경하는 과정 1,741.601 => 1741601
df_m_2023.iloc[0] = df_m_2023.iloc[0].str.replace(',','').astype(int)


# 여자 연령별 전국 데이터 정의
df_w_2023 = pd.read_excel('202308_202308_연령별인구현황_월간.xlsx',skiprows=3, index_col='행정기관', usecols='B,AE:AO')
df_w_2023.columns = df_m_2023.columns

df_w_2023.iloc[0] = df_w_2023.iloc[0].str.replace(',','').astype(int)
...
diff_cols= df_m_2013.columns
dif_m = df_m_2023.iloc[0] - df_m_2013.iloc[0]
dif_w = df_w_2023.iloc[0] - df_w_2013.iloc[0]
...
plt.title("2013~2023 대한민국 인구차 피라미드")
plt.barh(diff_cols,-dif_m // 1000) # barh(column, 데이터): 가로 막대,
plt.barh(diff_cols,dif_w // 1000)
plt.savefig('2013~2023 대한민국 인구차 피라미드.png', dpi = 100)
plt.show()
