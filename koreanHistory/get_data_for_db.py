    ...
    if response.status_code == 200:
        ...
        for row in rows:
            ...
            if len(columns) == 3:
                ...
                if '원종' in king_name:
                    print(f"king_name:{king_name},year_text:{year_text}")
                ...
                try:
                    for yt in year_text.strip().split('.'):
                        ...
                        if king_name in korean_dynasty_dict:
                            korean_dynasty_dict[king_name].append({'year': year, 'event': event})
                        else:
                            korean_dynasty_dict[king_name] = [{'year': year, 'event': event}]
                except ValueError:
                    continue
